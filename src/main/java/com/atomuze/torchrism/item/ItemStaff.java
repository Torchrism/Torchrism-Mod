...
public class ItemStaff extends net.minecraft.item.ItemTool {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if(player.isSneaking() && worldIn.getBlockState(pos).getBlock() == Blocks.TORCH) {
			...
			return EnumActionResult.SUCCESS;
		} else if (worldIn.getBlockState(pos).getBlock().isReplaceable(worldIn, pos) && worldIn.getBlockState(pos.down()).isNormalCube() && nbt.getInteger("count") > 0) {
				worldIn.setBlockState(pos, Blocks.TORCH.getDefaultState());
				addNbtCount("count", 1);
				return EnumActionResult.SUCCESS;
		} else if (worldIn.getBlockState(pos).isNormalCube() && facing != EnumFacing.DOWN && nbt.getInteger("count") > 0) {
			addNbtCount("count", -1);
