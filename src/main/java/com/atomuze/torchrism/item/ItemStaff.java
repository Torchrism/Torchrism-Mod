...
public class ItemStaff extends net.minecraft.item.ItemTool {
	...
	public ItemStaff(ToolMaterial material, String name) {
		super(0, 0, material, EFFECTIVE_ON);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if(nbt.hasKey("count")) {
			nbttaglist = (NBTTagList) nbt.getTag("count");
			System.out.println("hasKey");
		}else {
			nbttaglist = new NBTTagList();
		    nbt.setTag("count", nbttaglist);
		    stack.setTagCompound(nbt);
			nbt.setInteger("count", 1);
			System.out.println("nhasKey");
		}
		
		int a = nbt.getInteger("count");
		nbt.setInteger("count", a + 1);
